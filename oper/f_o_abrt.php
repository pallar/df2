...
<div style='height:59px; $thead_style'>"; else if ( $nosession==1 ) echo "<br><br>";
	...
	if ( $div_hide!=1 ) {
		...
		echo "
		<td $ljust colspan='2'><font color='#0'>".$ged['Group'].":&nbsp;</font><font color='#aaa'>".$row[3]."</font><br><font color='#0'>".$ged['Number'].":&nbsp;</font><font color='#aaa'>".$row[1]."</font><br><font color='#0'>".$ged['Nick'].":&nbsp;</font><font color='#aaa'>".StrCutLen( $row[2], 59 )."</font></td>
	</tr>
	<tr $cjust class='st_title2' style='height:27px'>";
	}
	echo "
...
<div style='height:323px; $tbody_style'>
	...
	while ( $row=mysql_fetch_row( $res )) {
		...
		<td $cjust rowspan='2' title='".StrCutLen( $row[3], 59 )."' width='$td2w'>".StrCutLen( $row[3], 7 )."</td>
