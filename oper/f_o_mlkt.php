...
<div style='height:59px; $thead_style'>"; else echo "<br><br>";
	...
	if ( $div_hide==1 ) {
		...
		echo "
		<td $ljust colspan='6'><font color='#0'>".$ged['Group'].":&nbsp;</font><font color='#aaa'>".$row[3]."</font><br><font color='#0'>".$ged['Number'].":&nbsp;</font><font color='#aaa'>".$row[1]."</font><br><font color='#0'>".$ged['Nick'].":&nbsp;</font><font color='#aaa'>".StrCutLen( $row[2], 59 )."</font></td>
	</tr>
	<tr $cjust class='st_title2' style='height:27px'>";
	} else echo "
...
<div style='height:323px; $tbody_style'>
	...
	while ( $row=mysql_fetch_row( $res )) {
		...
		<td $cjust height='22px' title='".StrCutLen( $row[3], 59)."' width='$td2w'>".StrCutLen( $row[3], 7 )."</td>
		<td $rjust title='".$cownum_div.$row[1].$cownum_div1."' width='$td3w'><b>".$cownum_div.StrCutLen( $row[1], 9 ).$cownum_div1."</b></td>
		<td $cjust title='".StrCutLen( $row[2], 59 ) ."' width='$td4w'>".StrCutLen( $row[2], 11 )."</td>";
			echo "
