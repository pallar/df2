...
if ( $cow_hist==0 ) include( "../reports/fhead.php" );
...
//while ( $yc<=$yl+1 ) {
//	$dbt=Int2StrZ( $yc, 4 ).Int2StrZ( $mc, 2 )."_o";
...
//		include( "f_jselo.php" );
		if ( $error==0 ) { while ( $row=mysql_fetch_row( $res )) {
			...
			if ( $odt>$yl1+$ml1+$dl );
				...
				if ( $filt_cowid==-1 ) $cownum="<a href='../".$hRep['o']."?filt_cowid=$cowid&title=".$ged['RHM000hist.'].":&nbsp;".$php_mm["_com_animal_cap"]."&nbsp;".$ged['No.']."&nbsp;$cownum,&nbsp;".$ged['nick']."&nbsp;$cownick'>".$cownum."</a>";
				$cowoperdes=$row[6];
...
if ( $filt_cowid>0 & (( $outsele_!=-1 & $outsele_==2048 ) | ( $outsele_==-1 ))) {
	$res=mysql_query( "SELECT b_date, cow_num, nick, comments, modif_date, modif_time, id
	 FROM $cows WHERE mth_id=$filt_cowid", $db ) or die( mysql_error());
	while ( $row=mysql_fetch_row( $res )) {
			...
			if ( $odt>$yl1+$ml1+$dl );
				...
				if ( $opertype*1==-1 ) {
					$tmp=$tmp."<td>".$ged['daughter']."&nbsp;".$cowoperdes."</td>";
				}
